...
/*   Updated: 2021/11/16 16:19:34 by yang             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sort_a(t_info *st_info)
{
	...
	while (!(temp_a->content < temp_a->next->content && temp_a->next->content < temp_a->prev->content))
	{
		if (temp_a->content > temp_a->prev->content && temp_a->content > temp_a->next->content)
...
void	sort_b(t_info *st_info)
{
	...
	while (!(temp_b->content > temp_b->next->content && temp_b->content > temp_b->prev->content))
	{
		if (temp_b->content < temp_b->prev->content && temp_b->content < temp_b->next->content)
...
void	sort_rotate_a(t_info *st_info)
{
	...
	while (st_info->st_a->rotate_count ||
		!(temp_a->content < temp_a->next->content && temp_a->next->content < temp_a->next->next->content))
	{
...
void	sort_rotate_b(t_info *st_info)
{
	...
	while (st_info->st_b->rotate_count ||
		!(temp_b->content < temp_b->next->content && temp_b->next->content < temp_b->next->next->content))
	{
